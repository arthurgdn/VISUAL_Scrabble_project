...
class Board:
    ...
    def extend_right(self, start_node, square_row, square_col, rack, word, squares, dist_from_anchor):
        ...
        if not square.letter:
            if start_node.is_terminal:
                self.score_word(word, squares, dist_from_anchor)
            for letter in start_node.children:
                ...
                if letter in rack:
                    ...
                    continue
                if letter in rack and self.cross_check(letter, square):
                    ...
                    self.extend_right(new_node, square_row, square_col + 1, new_rack, new_word, new_squares,
                                       dist_from_anchor)
        else:
    ...
    def left_part(self, start_node, anchor_square_row, anchor_square_col, rack, word, squares, limit,
        ...
        if potential_square.letter:
            return
        self.extend_right(start_node, anchor_square_row, anchor_square_col, rack, word, squares, dist_from_anchor)
        ...
        if limit > 0:
            for letter in start_node.children:
                ...
                self.left_part(new_node, anchor_square_row, anchor_square_col, new_rack, new_word, new_squares,
    ...
    def cross_check(self, letter, square):
        square.check_switch(self.is_transpose)
        chr_val = 65
    ...
    def get_all_words(self, square_row, square_col, rack):
        ...
        self.extend_right(self.dawg_root, square_row, square_col, rack, "", [], 0)
        ...
        for i, letter in enumerate(rack):
            ...
            self.left_part(self.dawg_root, square_row, square_col - 1, temp_rack, "", [], 6, 1)
        ...
        for i in range(len(self.board) - 1):
            for j in range(len(self.board[i]) - 1):
                ...
                if letter != ' ' or letter != '_':
                    self.board[i][j].letter = letter
                else:
                    self.board[i][j].letter = None
        self.update_cross_checks()
